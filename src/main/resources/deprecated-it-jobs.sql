...
WHERE (expireddate < curdate() AND expireddate >= SUBDATE(curdate(), INTERVAL 1 DAY ))
        OR
      (j.lastupdateddate BETWEEN '%s' AND '%s' AND NOT (isapproved = 1 AND isactive = 1 AND iscompleted = 1 AND isdeleted = 0 AND isUnpaidDisable <> 1))
GROUP BY j.jobid;
